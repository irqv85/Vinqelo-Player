...
class SmartPlaylistsPage(QWidget):
    ...
    def refresh(self) -> None:
        ...
        self._add_playlist("Global · Más escuchadas", "global", None)
        self._add_playlist("New · Últimos 30 días", "new", None)
        header = QListWidgetItem("POR ARTISTA")
        ...
        for artist in self.database.get_smart_playlist_artists():
            ...
            item.setToolTip(
                f'{format_listening_time(artist["listen_seconds"])} escuchados'
            )
    ...
    def _playlist_changed(self, current: QListWidgetItem | None, _previous: QListWidgetItem | None) -> None:
        ...
        self.heading.setText(
            f"{current.text()} · {len(self._rows)} pistas · "
            f"{_total_duration(total_duration)}"
        )
        self._fill_tracks()
    ...
    def _fill_tracks(self) -> None:
        self.tracks.clear()
        if not self._rows:
            empty = QTreeWidgetItem(["Aún no hay escuchas suficientes para esta lista.", "", "", ""])
